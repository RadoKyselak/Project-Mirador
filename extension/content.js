// content.js: listens for user selection (double-click or keyboard shortcut extension could be added later)
document.addEventListener("mouseup", async (e) => {
  const selection = window.getSelection().toString().trim();
  if (!selection) return;
  // Lightweight UX: small transient tooltip asking to verify
  ...
  prompt.onclick = () => {
    cleanup();
    // send message to background/service worker to open popup & call backend
    chrome.runtime.sendMessage({type: "verify", claim: selection});
  };

  // remove prompt after 4 seconds
